...
def calculate_ig(model, test_loader, num_classes=2, device="cuda", save_path=None):
    ...
    for batch_idx, batch in enumerate(test_loader):
       ...
       # print(f"labels {labels}")
        ...
        attributions = ig.attribute(inputs, target=None)
        ...
        ig_results.append(attributions.detach().cpu().numpy())

    # Concatenate all results
    ig_results = np.concatenate(ig_results, axis=0)

    if save_path:
        np.save(save_path, ig_results)
